...
public class ServerRunning implements MethodRule, RestTemplateHolder {
	...
	public Statement apply(final Statement base, FrameworkMethod method, Object target) {
		...
		try {
			client.getForEntity(new UriTemplate(getUrl("/oauth2/login.jsp")).toString(), String.class);
			online = true;
		...
		postForStatus(savedClient, "/oauth2/oauth/uncache_approvals",
		...
		return new Statement() {
			...
			public void evaluate() throws Throwable {
				try {
					...
					postForStatus(savedClient, "/oauth2/oauth/cache_approvals",
							new LinkedMultiValueMap<String, String>());
				}
