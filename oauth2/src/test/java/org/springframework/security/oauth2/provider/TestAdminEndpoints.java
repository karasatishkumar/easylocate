...
public class TestAdminEndpoints {
	...
	public void testListTokensByUser() throws Exception {
		ResponseEntity<String> result = serverRunning.getForString("/oauth2/oauth/users/marissa/tokens");
	...
	public void testRevokeTokenByUser() throws Exception {
		...
		assertEquals(
				...
				serverRunning
						.getRestTemplate()
						.exchange(serverRunning.getUrl("/oauth2/oauth/users/{user}/tokens/{token}"),
								HttpMethod.DELETE, request, Void.class, "marissa", token.getValue()).getStatusCode());

		ResponseEntity<String> result = serverRunning.getForString("/oauth2/oauth/users/marissa/tokens", headers);
	...
	public void testCannotListTokensOfAnotherUser() throws Exception {
		assertEquals(HttpStatus.FORBIDDEN, serverRunning.getStatusCode("/oauth2/oauth/users/foo/tokens"));
	}
	...
	public void testListTokensByClient() throws Exception {
		ResponseEntity<String> result = serverRunning
				.getForString("/oauth2/oauth/clients/my-client-with-registered-redirect/tokens");
		assertEquals(HttpStatus.OK, result.getStatusCode());
		assertTrue(result.getBody().contains(context.getAccessToken().getValue()));
	}
	...
	public void testUserCannotListTokensOfClient() throws Exception {
		assertEquals(HttpStatus.FORBIDDEN,
				serverRunning.getStatusCode("/oauth2/oauth/clients/my-client-with-registered-redirect/tokens"));
	}
