...
public class TestResourceOwnerPasswordProvider {
	...
	public void testUnauthenticated() throws Exception {
		...
		assertEquals(HttpStatus.UNAUTHORIZED, serverRunning.getStatusCode("/oauth2/photos?format=json"));
	}

	@Test
	public void testUnauthenticatedErrorMessage() throws Exception {
		HttpHeaders headers = new HttpHeaders();
		ResponseEntity<Void> response = serverRunning.getForResponse("/oauth2/photos?format=json", headers);
	...
	public void testTokenObtainedWithHeaderAuthentication() throws Exception {
		assertEquals(HttpStatus.OK, serverRunning.getStatusCode("/oauth2/photos?format=json"));
	...
	public void testTokenNotGrantedIfSecretNotProvided() throws Exception {
		try {
			...
			assertTrue(values.get(0).contains("Basic realm=\"oauth2/client\""));
		}
	}
	...
	public void testUserMessageIsProtectedResource() throws Exception {
		assertEquals(HttpStatus.UNAUTHORIZED, serverRunning.getStatusCode("/oauth2/photos/user/message"));
	}

	@Test
	@OAuth2ContextConfiguration(ResourceOwnerWithTrustedClient.class)
	public void testClientRoleBasedSecurity() throws Exception {
		assertEquals(HttpStatus.OK, serverRunning.getStatusCode("/oauth2/photos/user/message"));
	}
	...
	public void testUnsupportedMediaType() throws Exception {
		...
		// story.
		assertEquals(HttpStatus.OK, serverRunning.getStatusCode("/oauth2/photos/user/message", headers));
	}
	...
	public void testUnsupportedMediaTypeWithInvalidToken() throws Exception {
		...
		assertEquals(HttpStatus.NOT_ACCEPTABLE, serverRunning.getStatusCode("/oauth2/photos/user/message", headers));
	}
	...
	public void testMissingGrantType() throws Exception {
		...
		ResponseEntity<String> response = serverRunning.getForString("/oauth2/oauth/token", headers);
	...
	static class ResourceOwner extends ResourceOwnerPasswordResourceDetails {
		public ResourceOwner(Object target) {
			...
			setAccessTokenUri(test.serverRunning.getUrl("/oauth2/oauth/token"));
		}
	}
